...
describe('QuizzesModule', () => {
  ...
  it('lists quiz submissions using envelope pagination', async () => {
    vi.spyOn(client, 'paginateEnvelope').mockResolvedValueOnce([
      { id: 1, quiz_id: 1, user_id: 10, submission_id: 100, attempt: 1, score: 85, kept_score: 85, workflow_state: 'complete' },
    ])
  ...
  it('lists quiz questions', async () => {
    vi.spyOn(client, 'paginate').mockResolvedValueOnce([
      { id: 1, quiz_id: 1, position: 1, question_text: 'What is 2+2?', question_type: 'multiple_choice_question', points_possible: 5 },
    ])
    const result = await quizzes.listQuestions(100, 1)
    expect(result).toHaveLength(1)
    expect(client.paginate).toHaveBeenCalledWith(
      '/api/v1/courses/100/quizzes/1/questions',
    )
  })
