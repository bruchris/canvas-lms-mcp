...
| Every OAuth and `/mcp` request is **500** but `/health` is still 200  | The store rejected the pre-registered clients at startup — look for `Failed to seed pre-registered OAuth clients` on stderr. Usually a read-only store file or directory, or a full disk. Fix the permissions: the next request retries and the running process recovers, no restart needed. `/health` does not cover this, so do not rely on it alone to detect it. |
| Every user gets **503** `temporarily_unavailable` at once            | Look for `Canvas rejected this server's own OAuth request` on stderr. A wrong `CANVAS_OAUTH_CLIENT_ID` / `CANVAS_OAUTH_CLIENT_SECRET` (`error=invalid_client`) or a Developer Key redirect URI that is not exactly `<issuer>/oauth/canvas/callback` (`error=invalid_request`). Grants are deliberately left alone, so fixing the config restores every user without a re-login. |
