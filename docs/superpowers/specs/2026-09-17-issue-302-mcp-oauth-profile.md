...
8. Canvas access token expiring within 60 s ⇒ refresh via `POST /login/oauth2/token` (`grant_type=refresh_token`), deduplicated per grant so concurrent requests share one refresh. ~~Canvas answers 4xx ⇒~~ ~~**Corrected 2026-09-20 (QA S2):** Canvas answers 400 or 401 ⇒~~ **Corrected again 2026-09-20 (QA W2 on PR #356):** Canvas answers with `{"error": "invalid_grant"}`, **or** an unreadable 400 ⇒ the Canvas authorization is gone: the grant is revoked, its tokens deleted, and the client gets **401** `invalid_token` — the "revoked at Canvas" path. Every other failure, whatever the status, ⇒ **503** `temporarily_unavailable`, grant untouched.

   Two rounds of narrowing, each removing a way to log every user out at once. The original "any 4xx" rule made a 403 from a WAF, a 408, or a 429 during a rate-limit episode revoke every active user's grant — and revoke it *at Canvas*, so re-login was the only recovery. The S2 replacement still counted 401, but Canvas's own taxonomy (`lib/canvas/oauth/request_error.rb`, master `1c9f0bb8013e`, read 2026-09-20) reserves 401 for `{"error": "invalid_client"}` — `invalid_client_id` and `invalid_client_secret` — while every `invalid_grant` variant takes the default 400. So one rotated or mistyped `CANVAS_OAUTH_CLIENT_SECRET` answered 401 for everyone and would have revoked every grant. The code Canvas sends, not the status, decides; `invalid_client` and `invalid_request` are logged as the server misconfigurations they are. `error_description` is still never surfaced.
