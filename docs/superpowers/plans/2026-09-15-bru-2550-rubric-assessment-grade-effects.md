...
- **Shape**: one implementation PR, `fix(rubrics): …` (commit type is open question Q1)
...
- Keep `submitAssessment` **unchanged** and mark it deprecated. It is public library API via the `./canvas` export:
   ...
   * stability; see BRU-2550. Removal is open question Q2.
   */
  ```
...
Messages use the templates in §3.6 verbatim. **They must not contain the substrings `fetch`, `network`, `socket`, `dns`, `enotfound`, `econnrefused`, `econnreset` or `etimedout`**. Step 4 makes this moot, but keep the templates clean anyway.
...
- **K7 (late policy)**: `apply` with after `{score: 8, entered_score: 9, points_deducted: 1}` and rubric 9 → `score_changed`, not `GRADEBOOK_NOT_UPDATED`.
...
**Test**: `formatError(new ToolOutcomeError('network fetch failed after the write; observed score 9'))` returns that exact string. **Injection check (quote it in the PR)**: move the branch below the generic `Error` branch and confirm that exactly this test fails.
...
- **`gradeWrite`**, one of three modes:
  - `'canvas'` (default) replicates the `update_artifact` gates (probe J);
...
- **H6**: refusals through the full handler. Run each of `USE_FOR_GRADING_ON`, `USE_FOR_GRADING_OFF`, `CHECKPOINTS` (with `checkpointsFeature: true`), `NO_RUBRIC`, `MODERATED`, `GROUP_GRADED`, `EXCUSED`, `NO_POINTS`, `MISSING_CRITERIA` and `UNKNOWN_CRITERIA`. Each asserts zero PUTs, plus one control per row that PUTs.
...
**RED check**: before implementing Steps 3–5, run the new files. Every test except H14 and the renamed deprecated-client test must be red. If the red count is lower than the number of tests written minus 2, find the vacuous ones before continuing.
...
- **Body**:
  ...
  - list the RED tests vs the characterization tests (H14 and the renamed client test);
  - include the injection results and the dist probe output;
  - add the #327 sequencing note.
